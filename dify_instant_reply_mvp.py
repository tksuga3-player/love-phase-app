...
user_consultation = st.text_area(
    ...
    height=250,
    placeholder="例：マチアプで出会ったアラサーの女性。2回目のデートの帰り道で意を決して告白したら「今はそういうのは考えられない」と強めに言われてそそくさと解散した。デート中、彼女は笑ってくれてたし、会話は盛り上がっていたはずなのに何が悪かったのか分からない。もう少し会う回数を重ねてからすべきだったのか。次のデートに誘ったら「今は仕事が忙しい時期だから」と言われる。脈がないなら引くべきなのか。"
)
