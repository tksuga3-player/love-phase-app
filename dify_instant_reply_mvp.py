...
# --- UI設計 ---
st.title(" 🧬 無料！ あなたの繁殖現在地")
st.markdown("""
恋愛でうまくいかなかったときの出来事や状況を記入ください。
ヒトの遺伝子や脳は狩猟採集時代のまま。進化心理学モデルで、あなたの今の恋愛現在地、ボトルネック、今やるべきこと、やらなくていいことを、その場で抽出します。
""")
...
user_consultation = st.text_area(
    "📝 そのときの出来事や状況を具体的に入力してください",
    height=250,
    placeholder="例：私は学生時代から陰キャ寄りで女性経験も少ない。やっとの思いでマチアプで出会ったアラサーの女性を逃したくない。2回目のデートの帰り道で意を決して告白したら「今はそういうのは考えられない」と強めに言われてそそくさと解散した。デート中、彼女は笑ってくれてたし、会話は盛り上がっていたはずなのに何が悪かったのか分からない。もう少し会う回数を重ねてからすべきだったのか。次のデートに誘ったら「今は仕事が忙しい時期だから」と言われる。脈がないなら引くべきなのか。"
)
